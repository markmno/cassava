...
class LoadData():
    ...
    def get_dataloader(self, idx:Optional[int], bs:int, shuffle:bool=False, mode:Literal['train', 'test']='train', output_labels:bool=True):
        df = self.df.iloc[idx,:] if idx is not None else self.df
        transforms = self.get_train_transforms if mode == 'train' else self.get_test_transforms
        dataset = ImageDataset(df=df, root=self.root, transforms=transforms(), output_labels=output_labels)
        return DataLoader(dataset=dataset, batch_size=bs, shuffle=shuffle, num_workers= 4, pin_memory=True) 
...
from .base import InferenceHook, InferenceHookList, TrainHook, TrainHookList
...
class EpochProgress(TrainHook):
    ...
    def before_run(self) -> None:
        self.epoch = self.epoch_progress.add_task(
            "[purple]epoch", total=self.storage.get("epochs")
        )

    def after_epoch(self) -> None:
        self._best_component.update(self.storage.best_metric)
        self._stopping_counter.update(
            {"stopping counter": self.storage.get("stopping_counter")}
        )
...
class TrainStepProgress(TrainHook):
    ...
    def before_epoch(self) -> None:
        self.train_step = self.train_progress.add_task(
            "[red]train",
            total=self.storage.get("train_steps"),
            additional_columns=[self._train_loss_component],
        )
...
class EvalStepProgress(TrainHook):
    ...
    def before_epoch(self) -> None:
        self.eval_step = self.eval_progress.add_task(
            "[blue]eval ",
            total=self.storage.get("eval_steps"),
            additional_columns=[self._metric_component],
        )
...
class InferenceStepProgress(InferenceHook):
    ...
    def before_epoch(self) -> None:
        self.eval_step = self.eval_progress.add_task(
            "[blue]infer", total=self.storage.get("eval_steps")
        )
...
class InferenceFoldProgress(InferenceHook):
    ...
    def get_progress(self):
        self.epoch_progress = Progress(
            SpinnerColumn(spinner_name="arrow3"),
            TextColumn("fold [{task.completed}/[cyan]{task.total}[white]]"),
        )
        return self.epoch_progress

    def before_run(self) -> None:
        self.epoch = self.epoch_progress.add_task(
            "[purple]fold", total=self.storage.get("folds")
        )

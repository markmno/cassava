...
class BiTemperedLogisticLoss(nn.Module):
    ...
    def compute_normalization_fixed_point(self, activations):
        ...
        while i < self.num_iters:
            i += 1
            logt_partition = torch.sum(self.tempered_exp(normalized_activations, self.t2), dim=-1).view(-1, 1)
    ...
    def forward(self, activations, labels):
        ...
        if self.label_smoothing > 0:
            ...
            labels_onehot = ( 1 - self.label_smoothing * num_classes / (num_classes - 1) ) * labels_onehot + self.label_smoothing / (num_classes - 1)
        ...
        loss_values = temp1 - temp2 - labels_onehot.pow(2.0 - temperature1) / (2.0 - temperature1) + probabilities.pow(2.0 - temperature1) / (2.0 - temperature1)
